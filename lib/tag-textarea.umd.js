...
var printarea_default = /*#__PURE__*/function () {
  ...
  _createClass(_default, [{
    ...
    value: function init() {
      ...
      setTimeout(function () {
        _this.counter++;
        _this.settings.id = "printArea_".concat(_this.counter);

        var PrintAreaWindow = _this.getPrintWindow(); // 创建iframe


        _this.write(PrintAreaWindow.doc); // 写入内容
    ...
    value: function print(PAWindow) {
      ...
      var _loaded = function _loaded() {
        ...
        try {
          var box = document.getElementById(_this2.settings.id);

          var canvasList = _this2.elsdom.querySelectorAll('.canvasImg');
          ...
          if (_this2.settings.endCallback !== null) {
            _this2.settings.endCallback();
          }
        } catch (e) {
          console.log(e);
        }
    ...
    value: function getHead() {
      ...
      if (domStyle && domStyle.length > 0) {
        for (var i = 0; i < domStyle.length; i++) {
          try {
            ...
            console.error('+error:' + e); // console (domStyle[i].href +'error:'+ e);
          }
        }
      }
      ...
      var modeStyle = '<style>@page {size: portrait }</style>'; // console.log(this.settings.mode)
      ...
      return "<head><title>".concat(this.settings.popTitle, "</title>").concat(extraHead).concat(links, "<style type=\"text/css\">").concat(style, "</style>").concat(modeStyle, "</head>");
    }
  }, {
    key: "getBody",
    ...
    el.addEventListener('click', function () {
      vue.$nextTick(function () {
        if (typeof binding.value === 'string') {
          id = binding.value;
        } else if (typeof_typeof(binding.value) === 'object' && !!binding.value.id) {
          id = binding.value.id;
